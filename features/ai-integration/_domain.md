...
- [[multi-provider-llm-abstraction--from-browser-use]] — one `ainvoke(messages, output_format)` Protocol over OpenAI/Anthropic/Google/Groq/etc. with guaranteed structured output, each provider adapting differently (OpenAI json_schema; **Anthropic forced tool-use**; Gemini response_schema). The hand-rolled, LangChain-free counterpart to [[provider-agnostic-llm--from-llm-scraper]].
