...
- [[oklch-theme-palettes--from-carousel-generator]] — DaisyUI-style named themes reduced to a 3-color runtime palette; derives a readable foreground from any base color by interpolating toward black/white in OKLCH (direction chosen by a WCAG `isDark` test) via culori
- [[native-design-tokens--from-penpot]] — typed tokens in ordered sets, themes that select which sets are active, and a resolved view computed by merging active sets in order (later overrides earlier) then resolving `{path}` references; all token edits ride the file's change/undo system
