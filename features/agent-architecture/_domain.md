...
- [[agent-loop-recovery--from-browser-use]] — a production think->act->observe loop with layered recovery: nuanced failure counting, a two-stage forced-done (swap the output schema to done-only), soft loop detection (action-hash + page fingerprint), provider fallback, and browser-disconnect tolerance. "Inspired by coding agents."
- [[action-tool-registry--from-browser-use]] — `@tools.action` decorator + Pydantic-validated action space: derives a schema from the function signature, combines actions into a `RootModel[Union]` for the LLM's structured output, injects framework-only "special" params, domain-scopes tools, and substitutes `<secret>` placeholders (incl. live TOTP) at dispatch.
