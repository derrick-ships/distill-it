...
- [[cli-subprocess-agent--from-open-carrusel]] — drive an app by spawning the local Claude Code CLI as a one-shot subprocess per message (`-p --output-format stream-json --resume <id> --max-budget-usd`), relaying its NDJSON events to the browser as SSE; "tools" are just Bash/WebFetch + a REST API documented in the system prompt (no custom tool protocol); session continuity via `--resume`; cross-platform binary discovery
